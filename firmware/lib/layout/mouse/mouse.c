...
// single left click (?)
void key_functions__mouse_left_click(int8_t x, int8_t y) {
  usb__m__send(x, y, 0, 0, MOUSE_BTN1);
  usb__m__send(x, y, 0, 0, 0);
}

void key_functions__mouse_move(int8_t x, int8_t y) {
  usb__m__send(x, y, 0, 0, 0);
}
